...
def join_features(df_in):
    ...
    # Main road features - SIMPLE
    cols = [c for c in df_roads.columns.tolist() if \
        "ROAD_ID" in c or \
        "FLOW" in c or\
        "SPEED" in c or \
        "SHOULDER" in c or \
        ...
        "INTERSECT" in c or\
        "SHOULDER" in c or \
        "PASSENGER_SIDE_DISTANCE" in c]
    df_in = pd.merge(df_in, df_roads.drop("BLOCK", axis=1)[cols], on="ROAD_ID", how="left")
...
df_train_class = make_class_dataset()
...
model_cv_class1 = build_block_cv(df_train_class, exclude_cols_class, response_class, eval_metric_class, params_class)
print ">>> Classification (frequency) models (x1) built in", round((time.time() - t1)/60.,2),"minutes"
...
y = (df_train_class["COST"] > 0).astype(np.int).mean() # rate of 1s in model
unwind = 1/(1+(1/t-1)/(1/y-1)*(1/df_test2["freq"]-1))
df_test2["freq_unwind"] = unwind

# Combine
df_test2["COST"] = df_test2["freq_unwind"] * df_test2["sev"]
df_test2[["ID","COST"]].to_csv("predictions_simple.csv", index=False)
